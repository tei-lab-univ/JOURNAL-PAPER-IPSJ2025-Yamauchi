...
CDCS\cite{yamauchi:IPSJ2024}は，監視モデルの違反状態に該当するゲーム空間の状態を，全てひとつの状態に抽象化しつつゲーム空間構築することで，DCSの計算空間を削減する．
安全性ゲームを解く上で，ゲーム空間の違反状態から伸びる遷移は探索する必要がない．
そこで，ゲーム空間構築過程で複雑な違反状態の構築を回避することで，計算空間削減を実現する．
% この抽象化によって，構築されるゲーム空間の状態削減を実現した．
...
Algorithm\ref{algorithm:CDCS}では，Algorithm\ref{algorithm:basic_controller_synthesis}におけるParallel Composition，Modified Parallel Composition，Error State Abstractionの3つを一括で行うConsolidated Game Compositionを用いて，$E$と$R$から直接，違反状態が圧縮されたゲーム空間$g^*$を構築する（1行目）．その後，basic DCSと同様に$g^*$を入力としてSafety Game Solvingを用いて$c$を導出する．
...
\label{def:consolidated_game_composition}
    記号$\between$で表され，環境モデル群$E$と監視モデル群$R$に含まれる全てのLTSの集合 $E \cup R$を入力とし，違反状態が圧縮されたゲーム空間$g^*$を構築し，出力する．
...
このConsolidated Game Compositionを用いることによって，入力されたモデルの$s_{err}$が重ね合わされる全状態をひとつの$s_{err}$として圧縮しつつ，ゲーム空間$g'$を構築できる．これにより，$s_{err}$が重ね合わされる状態空間だけ計算空間が削減できる．
...
Algorithm\ref{algorithm:SDCS}では，$\Lambda$に則って，繰り返し部分合成（6-9行目）を適用することで制御器$c$を導出する．
このとき，$\Lambda$は以下で定義される．
...
SDCSにおける$\Lambda$は，Algorithm\ref{algorithm:huristic}によって導出できる（1行目）\cite{yamauchi:KBSE2024}．
